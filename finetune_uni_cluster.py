...
# --- Configuration OPTIMIZED for CPU/GPU Compatibility ---
...
if torch.cuda.is_available():
    ...
    # Only set CUDA environment if GPU is available
...
print(f"Device-compatible configuration:")
...
try:
    ...
    # Clear memory after loading (GPU only)
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
...
try:
    ...
    if torch.cuda.is_available():
        ...
        gc.collect()  # CPU garbage collection
...
# --- 3. Device-Compatible Fine-tuned Model Architecture ---
class FineTunedUNImodel(nn.Module):
    def __init__(self, base_uni_model, num_features, num_classes_out, use_mixed_precision=False):
        super().__init__()
...
# Instantiate model with mixed precision only for GPU
use_amp = torch.cuda.is_available()
model_to_finetune = FineTunedUNImodel(base_model, feature_dim, NUM_CLASSES, use_mixed_precision=use_amp)
model_to_finetune = model_to_finetune.to(DEVICE)

# --- 4. Device-Compatible Data Loaders ---
...
# --- 5. Training Setup with Device Compatibility ---
...
# --- 6. Device-Compatible Training Loop ---
best_val_accuracy = 0.0
best_model_path = "/cluster/projects/bhatgroup/response_to_steroid/finetuned_uni_model_best.pth"
patience_counter = 0
max_patience = 4

print(f"\nStarting device-compatible fine-tuning for {NUM_EPOCHS} epochs on {DEVICE}...")
...
for epoch in range(NUM_EPOCHS):
    ...
    for batch_idx, (images, labels) in enumerate(progress_bar):
        # Device-compatible tensor transfer
        if torch.cuda.is_available():
        ...
        # Device-compatible forward pass
        ...
        # Clear cache periodically (GPU only)
        if batch_idx % 20 == 0:
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
            else:
...
# --- 8. Extract Features (Device-Compatible) ---
...
feature_batch_size = 4 if torch.cuda.is_available() else 2  # Even smaller for CPU
...
# Move base model to CPU to free GPU memory
base_model.cpu()
if torch.cuda.is_available():
    torch.cuda.empty_cache()
gc.collect()
