...
# Set up logging
log_file = "infold_hypertuned_multimodal_fusion.log"
...
def get_hyperparameter_grids():
    ...
    fusion_param_grids = {
        'weights': np.arange(0.5, 13/1549),  # Clinical weight (pathology weight = 1 - clinical_weight)
        'threshold': [0.5]  # Decision threshold
    }
    
    return clinical_param_grids, pathology_param_grids, fusion_param_grids

def tune_model_on_fold_data(model_class, param_grid, X_train, y_train, scale_features=False, cv=5):
    """Tune hyperparameters on the training data of a fold using cross-validation"""
...
def main():
    log_output("=== Starting In-Fold Hyperparameter Tuned 5-Fold Cross-Validation for Multimodal Fusion ===")
    ...
    log_output("=== Hyperparameter tuning performed INSIDE EACH FOLD on training data only (4/5) ===")
    ...
    # Initialize data structures to track model performance and parameters across folds
    ...
    # Track best parameters for each fold
    clinical_best_params = defaultdict(list)
    pathology_best_params = defaultdict(list) 
    fusion_best_params = defaultdict(list)
    
    # ------ 5. Perform Cross-Validation with In-Fold Hyperparameter Tuning ------
    ...
    for fold, (train_idx_clinical, test_idx_clinical) in enumerate(kf_clinical.split(X_clinical)):
        ...
        # --- 5.2. Split Pathology Data Based on Patient IDs ---
        ...
        # --- 5.3. Tune Clinical Models on Training Data and Test ---
        log_output("\n--- Tuning and Training Clinical Models on Fold Training Data ---")
        
        clinical_results = {}
        for name, config in clinical_model_configs.items():
            log_output(f"\nTuning Clinical {name.upper()} on fold {fold+1} training data...")
            
            # Tune hyperparameters using only training data
            best_params, _ = tune_model_on_fold_data(
                config['model'],
                clinical_param_grids[name],
                X_clinical_train,
                y_clinical_train,
                scale_features=config['scale'],
                cv=5  # Use 5-fold CV inside each training fold
            )
            
            # Store best parameters for this model in this fold
            clinical_best_params[name].append(best_params)
            log_output(f"Clinical {name.upper()} best params for fold {fold+1}: {best_params}")
            
            # Create model with best parameters
            model = config['model'](random_state=SEED, **best_params)
        ...
        # --- 5.4. Tune Pathology Models on Training Data and Test ---
        log_output("\n--- Tuning and Training Pathology Models on Fold Training Data ---")
        
        pathology_results = {}
        for name, config in pathology_model_configs.items():
            log_output(f"\nTuning Pathology {name.upper()} on fold {fold+1} training data...")
            ...
            # Store best parameters for this model in this fold
            pathology_best_params[name].append(best_params)
            log_output(f"Pathology {name.upper()} best params for fold {fold+1}: {best_params}")
        ...
        # --- 5.5. Aggregate Pathology Predictions at Slide Level ---
        ...
        # --- 5.6. Tune and Evaluate Fusion Models ---
        ...
        for clin_name, path_name in itertools.product(clinical_model_configs.keys(), pathology_model_configs.keys()):
            ...
            if len(common_patients) > 0:
                # Tune fusion parameters on common patients
                ...
                clinical_weight = best_fusion_params['weight']
                pathology_weight = 1.0 - clinical_weight
    ...
    # ------ 6. Calculate & Report Average Performance Across Folds ------
    
    # --- 6.1. Report Best Parameters Found in Each Fold ---
    log_output("\n=== Best Hyperparameters Found in Each Fold ===")
    
    log_output("\nClinical Models:")
    for name in clinical_model_configs.keys():
        log_output(f"{name.upper()}:")
        for fold, params in enumerate(clinical_best_params[name]):
            log_output(f"  Fold {fold+1}: {params}")
    
    log_output("\nPathology Models:")
    for name in pathology_model_configs.keys():
        log_output(f"{name.upper()}:")
        for fold, params in enumerate(pathology_best_params[name]):
            log_output(f"  Fold {fold+1}: {params}")
    ...
    for combo_name in fusion_best_params.keys():
        ...
        if params_list:
            ...
            for fold, params in enumerate(params_list):
                log_output(f"  Fold {fold+1}: {params}")
    
    # --- 6.2. Clinical Models Performance ---
    ...
    # --- 6.3. Pathology Models Performance (Patch Level) ---
    ...
    # --- 6.4. Pathology Models Performance (Slide Level) ---
    ...
    # Save results
    results_file = "infold_hypertuned_multimodal_performance.csv"
    all_results.to_csv(results_file, index=False)
    log_output(f"Results saved to {results_file}")
    
    # Save best parameters
    params_file = "infold_best_hyperparameters.pkl"
    best_params_dict = {
        'clinical': dict(clinical_best_params),
        'pathology': dict(pathology_best_params),
