...
def main():
    ...
    for fold, (train_idx_clinical, test_idx_clinical) in enumerate(kf_clinical.split(X_clinical)):
        ...
        # --- 4.2. Split Pathology Data Randomly ---
        log_output("Randomly splitting pathology data (patches may be mixed across train/test)...")
        
        # Calculate test size to match clinical data split ratio
        test_size = len(test_idx_clinical) / len(X_clinical)
        
        # Random split of pathology data without considering patient IDs
        path_indices = np.arange(len(X_pathology))
        train_indices, test_indices = train_test_split(
            path_indices, test_size=test_size, random_state=SEED+fold
        )
        
        # Create train and test datasets
        train_items = [(all_paths[i], y_pathology[i]) for i in train_indices]
        test_items = [(all_paths[i], y_pathology[i]) for i in test_indices]
        ...
        # Log information about patch distribution
        train_slides = set([os.path.basename(path).split('-')[1] if len(os.path.basename(path).split('-')) >= 2 else "unknown" 
                           for path, _ in train_items])
        test_slides = set([os.path.basename(path).split('-')[1] if len(os.path.basename(path).split('-')) >= 2 else "unknown" 
                          for path, _ in test_items])
        common_slides = train_slides.intersection(test_slides)
        
        log_output(f"Number of unique slides in train set: {len(train_slides)}")
        log_output(f"Number of unique slides in test set: {len(test_slides)}")
        log_output(f"Number of slides with patches in both train and test sets: {len(common_slides)}")
